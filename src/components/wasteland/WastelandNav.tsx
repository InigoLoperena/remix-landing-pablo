...
export const WastelandButton = ({
  ...
  const variants: Record<string, string> = {
    ...
    ghost: '!text-rust/60 hover:!text-rust font-inter',
  ...
  return (
    <button
      className={`relative px-8 py-3 font-inter font-bold transition-all duration-100 flex items-center justify-center gap-2 group uppercase tracking-widest ${variants[variant]} ${className}`}
      {...props}
    >
      {children}
    </button>
...
export const SectionHeading = ({
  ...
  <div className="mb-12">
    {badge && (
      <span className="inline-block px-2 py-1 bg-hazard-yellow text-rust text-[10px] font-inter font-bold mb-4 tracking-widest uppercase border-2 border-rust shadow-[2px_2px_0px_#3d2616]">
        {badge}
      </span>
    )}
    <h2 className="text-6xl md:text-8xl font-crimson mb-6 tracking-tight text-rust uppercase leading-[0.85] drop-shadow-[4px_4px_0px_rgba(61,38,22,0.3)]">
      {title}
    </h2>
    {subtitle && (
      <p className="text-rust/80 font-inter font-bold text-lg max-w-2xl leading-relaxed bg-parchment/60 p-2 border-l-4 border-toxic-green">{subtitle}</p>
    )}
  </div>
);
...
export const WastelandNavbar = () => (
  <header className="h-20 fixed top-0 left-0 right-0 z-50 border-b-4 border-rust px-10 flex items-center justify-between bg-rust text-parchment scrap-border">
    <div className="flex items-center gap-6">
      ...
      <span className="text-xs font-inter text-parchment/60 tracking-[0.2em] uppercase hidden lg:block">
        NYC / GREEN_HUNT
      </span>
    </div>
    <div className="flex items-center gap-8 text-[11px] font-inter tracking-widest font-bold">
      ...
      <button
        className="bg-parchment !text-rust px-4 py-2 border-2 border-rust pixel-shadow hover:bg-hazard-yellow transition-colors uppercase font-inter font-bold tracking-widest scale-90 active:translate-x-1 active:translate-y-1 active:shadow-none"
...
export const WastelandTicker = () => (
  <footer className="h-10 fixed bottom-0 left-0 right-0 z-50 rust-bar text-parchment flex items-center overflow-hidden whitespace-nowrap border-t-4 border-rust">
    <div className="flex animate-ticker font-inter font-bold text-sm uppercase tracking-tighter">
      {[...Array(4)].map((_, i) => (
        <React.Fragment key={i}>
          <span className="px-8 text-hazard-yellow">The asphalt is ours</span>
          <span className="px-8 text-toxic-green text-[10px]">#</span>
          <span className="px-8 font-inter">3,000,000 TONS WASTED ANNUALLY</span>
          <span className="px-8 text-toxic-green text-[10px]">#</span>
          <span className="px-8 text-parchment animate-flicker">Join the Scavenger Resistance</span>
          <span className="px-8 text-toxic-green text-[10px]">#</span>
          <span className="px-8 font-inter text-[10px] text-parchment/60">RECLAIM THE MAP</span>
          <span className="px-8 text-toxic-green text-[10px]">#</span>
        </React.Fragment>
      ))}
    </div>
